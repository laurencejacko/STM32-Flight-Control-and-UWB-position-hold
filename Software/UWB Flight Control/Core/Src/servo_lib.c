...
void servo_angle(servo_config* servo_config, uint32_t angle)
{
	if(0!= angle)
	{
		...
		__HAL_TIM_SET_COMPARE(servo_config->Timer_Handle,servo_config->Timer_Channel, angle );
	}

}

void stop_pwm(servo_config* servo_config){
	HAL_TIM_PWM_Stop(servo_config->Timer_Handle, servo_config->Timer_Channel);
}
