...
/* USER CODE BEGIN 1 */
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)  //串�?�接收中断回调函数
{


	if(huart == &huart6)   //判断串�?�2中断
	{
		disconnect_time = uwTick;
			RxBuf[RxBuf_Index++] = RxBuffer[0];			//接收�?�字节数�?�到缓冲区
			HandleByteReceived();

			if (rtos_on==1){
			CRSFtime = osKernelSysTick();
			}
