...
uint8_t CSP_QSPI_EnableMemoryMappedMode(void) {
	...
	sCommand.NbData = 0;
	sCommand.DummyCycles = 0;//could be 2
...
void QSPI_Init_Custom(bool eraseFlash){


	uint8_t SECTOR[0x800]; //4bytes
	uint32_t addrBegin = 0;
	uint8_t endMarker[16] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
