...
static void dshot_set_timer(dshot_type_e dshot_type) {
	...
	dshot_prescaler = lrintf(
			(float) timer_clock / dshot_choose_type(dshot_type) + 0.01f) - 1;
...
static void dshot_dma_start() {
	HAL_DMA_Start_IT(MOTOR_1_TIM->hdma[TIM_DMA_ID_CC4],
			(uint32_t) motor1_dmabuffer,
			(uint32_t) &MOTOR_1_TIM->Instance->CCR4, DSHOT_DMA_BUFFER_SIZE);
	HAL_DMA_Start_IT(MOTOR_2_TIM->hdma[TIM_DMA_ID_CC3],
			(uint32_t) motor2_dmabuffer,
			(uint32_t) &MOTOR_2_TIM->Instance->CCR3, DSHOT_DMA_BUFFER_SIZE);
	HAL_DMA_Start_IT(MOTOR_3_TIM->hdma[TIM_DMA_ID_CC1],
			(uint32_t) motor3_dmabuffer,
			(uint32_t) &MOTOR_3_TIM->Instance->CCR1, DSHOT_DMA_BUFFER_SIZE);
	HAL_DMA_Start_IT(MOTOR_4_TIM->hdma[TIM_DMA_ID_CC2],
			(uint32_t) motor4_dmabuffer,
