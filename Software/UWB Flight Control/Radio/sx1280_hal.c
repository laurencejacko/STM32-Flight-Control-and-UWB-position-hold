...
{
  HAL_SPI_TransmitReceive(&hspi2, (uint8_t*)txBuffer, (uint8_t *)rxBuffer, size, 0xffff);
}
void SpiIn( uint8_t *txBuffer, uint16_t size )
{
    
  if(HAL_SPI_Transmit(&hspi2, (uint8_t*)txBuffer, size, 0xffff) != HAL_OK){
	  HAL_Delay(100);
  }
...
void SX1280HalClearInstructionRam( void )
{
  ...
  //  SpiIn( halTxBuffer, halSize );
    HAL_SPI_Transmit(&hspi2, (uint8_t*)halTxBuffer, halSize, 0xffff);
