...
static uint8_t rx_data_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'W', 'A', 'V', 'E', 0x30, 0,0,0,0,0,0,0,0,0 };
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & DWT_INT_RXFCG_BIT_MASK)
                {
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                       ...
                       uint8_t pmessage[8];
                       sprintf(pmessage, "%2.6f", distance);

                        for (size_t i = 0; i < sizeof(double); i++){
                        	rx_data_msg[i+10] = pmessage[i];
                        }
                        ...
                        /* Write and send the response message. See NOTE 10 below.*/
                        rx_data_msg[ALL_MSG_SN_IDX] = frame_seq_nb;
                                      dwt_writetxdata(sizeof(rx_data_msg), rx_data_msg, 0); /* Zero offset in TX buffer. */
                                      dwt_writetxfctrl((sizeof(rx_data_msg) + FCS_LEN), 0, 0);          /* Zero offset in TX buffer, ranging. */
